...
class CoHereProvider(LLMInterface):
    ...
    def generate_text(self,prompt:str,chat_history:list=[],max_output_tokens:int=None,temperature:float=None):
        self.logger.info(f"Generating text using #CohereProvider: {self.generation_model_id}")
        if not self.client:
    ...
    def invoke(self, prompt: str, chat_history: list = [], max_output_tokens: int = None, temperature: float = None):
        """Alias for generate_text, satisfies LLMInterface abstract contract."""
    ...
    def bind_tools(self, tools: List[BaseTool]):
        """Store tools for potential use in future Cohere tool-calling support."""
        self.tools = tools
        return self

    async def stream(self, messages: list) -> AsyncIterator[str]:
        """Cohere is used for embeddings only — streaming is not supported."""
        raise NotImplementedError(
            "CoHereProvider does not support chat streaming. "
            "Use GeminiProvider or OpenAIProvider for the chat tiers."
        )
        # Required to make this an async generator (yield makes it one)
